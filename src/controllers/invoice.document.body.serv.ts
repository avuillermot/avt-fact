...
export class InvoiceBodyService {
    ...
    public async generateDetails(invoice: IInvoice): Promise<void> {
        ...
        let line: number = 2;
        let y = 230 + ((invoice.items.length + line) * lineHeight);
        this.document.rect(col5, 215 + ((i + line) * lineHeight), 515 - col4 - 20, lineHeight).lineWidth(0).stroke();

        this.document.fontSize(8).font(this.defaultFont)
            .text("TVA €", col6, y)
            .text(invoice.taxAmount.toString(), col7, y);

        line++;
        y = 230 + ((invoice.items.length + line) * lineHeight);
        this.document.rect(col5, 215 + ((i + line) * lineHeight), 515 - col4 - 20, lineHeight).lineWidth(0).stroke();

        this.document.fontSize(8).font(this.defaultFont)
            .text("TOTAL €", col6, y)
            .text(invoice.total.toString(), col7, y);
    }
}